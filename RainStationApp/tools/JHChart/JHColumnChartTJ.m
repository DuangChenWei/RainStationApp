...
@interface JHColumnChartTJ ()<CAAnimationDelegate>
...
@implementation JHColumnChartTJ
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _needXandYLine = YES;
        _isShowYLine = YES;
        _lineChartPathColor = ColorWithAlpha(0xffffff, 1);
        _lineChartValuePointColor = ColorWithAlpha(0xf56e4e, 1);
        _valumnTextColor=[UIColor blackColor];
    }
    return self;
...
//        _maxHeight += 4;
        ...
        shapeLayer.strokeColor = (_dashColor==nil?([UIColor whiteColor].CGColor):_dashColor.CGColor);
    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            itemsView.backgroundColor = (UIColor *)(_columnBGcolorsArr.count<arr.count?[UIColor clearColor]:_columnBGcolorsArr[j]);
...
//                    NSLog(@"+-+-+-%@",str);
                    ...
                    textLayer.alignmentMode = kCAAlignmentCenter;
                    textLayer.contentsScale = [UIScreen mainScreen].scale;
                    textLayer.foregroundColor = _valumnTextColor.CGColor;
